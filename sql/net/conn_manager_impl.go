...
// mgr represents a connection map.
type mgr struct {
...
func (cm *mgr) AddConn(c Conn) error {
	cm.mutex.Lock()
...
func (cm *mgr) UpdateConn(from Conn, to Conn) error {
	cm.mutex.Lock()
	defer cm.mutex.Unlock()
...
func (cm *mgr) LookupConnByUID(cid uint64) (Conn, bool) {
	cm.mutex.RLock()
	defer cm.mutex.RUnlock()
	c, ok := cm.uidMap[cid]
	return c, ok
...
func (cm *mgr) LookupConnByUUID(uuid uuid.UUID) (Conn, bool) {
	cm.mutex.RLock()
	defer cm.mutex.RUnlock()
	c, ok := cm.m[uuid]
	return c, ok
...
func (cm *mgr) RemoveConn(conn Conn) error {
	cm.mutex.Lock()
	defer cm.mutex.Unlock()
...
func (cm *mgr) RemoveConnByUID(cid uint64) {
...
// RemoveConnByUID deletes the specified connection by the connection ID.
func (cm *mgr) RemoveConnByUUID(uuid uuid.UUID) {
...
func (cm *mgr) Start() error {
	return nil
}

// Close closes the connection manager.
func (cm *mgr) Close() error {
	var errs error
	conns := cm.Conns()
